...
const readDog = (req, res) => {
  ...
  Dog.findByName(tempName, callback);
};
...
const setDogName = (req, res) => {
  if (!req.body.name || !req.body.breed || !req.body.age) {
    return res.status(400).json({ error: 'Name, breed and age are all required. Please try again.' });
  }
...
const updateLastDog = (req, res) => {
  ...
  let currBreed = lastDogAdded.breed;
  savePromise.then(() => res.json({ name: lastDogAdded.name, breed: currBreed, age: lastDogAdded.age }));
